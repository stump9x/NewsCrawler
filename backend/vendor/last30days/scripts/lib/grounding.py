...
def wigolo_search(
    query: str, date_range: tuple[str, str], config: dict, count: int = 8,
) -> tuple[list[dict], dict]:
    """Call Wigolo REST search from the last30days CLI subprocess."""
    import json
    import urllib.error
    import urllib.request

    base = (config.get("WIGOLO_URL") or "").strip().rstrip("/")
    if not base:
        raise RuntimeError("WIGOLO_URL is required when web_backend='wigolo'")
    token = (config.get("WIGOLO_API_KEY") or config.get("WIGOLO_API_TOKEN") or "").strip()
    # Prefer query arrays + deep depth (Wigolo quality levers).
    q_payload: str | list[str]
    if isinstance(query, (list, tuple)):
        q_payload = [str(q).strip() for q in query if str(q).strip()][:8]
        if not q_payload:
            raise RuntimeError("empty wigolo query")
        if len(q_payload) == 1:
            q_payload = q_payload[0]
    else:
        q_payload = str(query or "").strip()
    depth = str(config.get("WIGOLO_SEARCH_DEPTH") or "deep").strip() or "deep"
    body = {
        "query": q_payload,
        "max_results": max(1, min(int(count or 8), 20)),
        "category": "news",
        "search_depth": depth,
        "time_range": "month",
    }
    try:
        from_d = datetime.fromisoformat(date_range[0]).date()
        to_d = datetime.fromisoformat(date_range[1]).date()
    ...
    try:
        ...
        raise RuntimeError(f"wigolo HTTP {exc.code}") from exc
    except Exception as exc:
        raise RuntimeError(f"wigolo request failed: {exc}") from exc
    ...
    for index, row in enumerate(rows or []):
        ...
        snippet = str(row.get("snippet") or row.get("excerpt") or row.get("content") or "")
        published = ""
        fresh = row.get("freshness_signal")
        ...
        items.append({
            ...
            "relevance": float(score or 0.5),
            "source": "web",
            "why_relevant": "Wigolo multi-engine web search",
        })
    artifact = {
        "label": "wigolo",
...
def web_search(
    ...
    artifact: dict = {}
    if backend == "wigolo":
        items, artifact = wigolo_search(query, date_range, config)
    elif backend == "brave":
