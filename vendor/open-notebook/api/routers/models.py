...
async def create_model(model_data: ModelCreate):
    ...
    try:
        ...
        # NewsCrawler Notebook AI: block TTS/STT model registration by default.
        if os.environ.get("OPEN_NOTEBOOK_DISABLE_SPEECH", "1").lower() in (
            "1",
            "true",
            "yes",
            "on",
        ) and model_data.type in ("text_to_speech", "speech_to_text"):
            raise HTTPException(
                status_code=410,
                detail="Speech models (TTS/STT) are disabled in this deployment",
            )
